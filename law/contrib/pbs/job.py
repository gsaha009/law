...
"""
Pbs job manager. See https://albertsk.org/wp-content/uploads/2011/12/pbs.pdf
"""

__all__ = ["PbsJobManager", "PbsJobFileFactory"]
...
class PbsJobManager(BaseJobManager):

    # chunking settings
    chunk_size_submit = 0
    chunk_size_cancel = _cfg.get_expanded_int("job", "pbs_chunk_size_cancel")
    chunk_size_query = _cfg.get_expanded_int("job", "pbs_chunk_size_query")

    submission_cre = re.compile(r"^Submitted batch job (\d+)$")

    squeue_format = r"PBS_JOBID,PBS_O_HOST,PBS_ARRAYID"
    squeue_cre = re.compile(r"^\s*(\d+)\s+([^\s]+)$")

    sacct_format = r"PBS_JOBID,PBS_JOBNAME,PBS_QUEUE,PBS_O_LOGNAME"
    ...
    def cleanup(self, *args, **kwargs):
        raise NotImplementedError("PbsJobManager.cleanup is not implemented")

    def cleanup_batch(self, *args, **kwargs):
        raise NotImplementedError("PbsJobManager.cleanup_batch is not implemented")
    ...
    def submit(self, job_file, partition=None, retries=0, retry_delay=3, silent=False):
        ...
        while True:
            # run the command
            logger.debug("submit pbs job with command '{}'".format(cmd))
            ...
            if code == 0:
                ...
                for line in out.strip().split("\n"):
                    ...
                    code = 1
                    err = "cannot parse pbs job id(s) from output:\n{}".format(out)

            # retry or done?
            if code == 0:
                return job_id

            logger.debug("submission of pbs job '{}' failed with code {}:\n{}".format(
                job_file, code, err))
            ...
            raise Exception("submission of pbs job '{}' failed:\n{}".format(
                job_file, err))
    ...
    def cancel(self, job_id, partition=None, silent=False):
        ...
        # run it
        logger.debug("cancel pbs job(s) with command '{}'".format(cmd))
        code, out, err = interruptable_popen(cmd, shell=True, executable="/bin/bash",
            stdout=subprocess.PIPE, stderr=subprocess.PIPE)

        # check success
        if code != 0 and not silent:
            raise Exception("cancellation of pbs job(s) '{}' failed with code {}:\n{}".format(
                job_id, code, err))
    ...
    def query(self, job_id, partition=None, silent=False):
        ...
        logger.debug("query pbs job(s) with command '{}'".format(cmd))
        code, out, err = interruptable_popen(cmd, shell=True, executable="/bin/bash",
            stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        ...
        if code != 0 and "invalid job id specified" in err.lower():
            ...
            if code != 0:
                if silent:
                    ...
                    raise Exception("queue query of pbs job(s) '{}' failed with code {}:"
        ...
        if missing_ids:
            ...
            logger.debug("query pbs accounting history with command '{}'".format(cmd))
            code, out, err = interruptable_popen(cmd, shell=True, executable="/bin/bash",
                stdout=subprocess.PIPE, stderr=subprocess.PIPE)
            ...
            if code != 0:
                if silent:
                    ...
                    raise Exception("accounting query of pbs job(s) '{}' failed with code {}:"
                        "\n{}".format(job_id, code, err))
            ...
            if _job_id not in query_data:
                if not chunking:
                    if silent:
                        ...
                        raise Exception("pbs job(s) '{}' not found in query response".format(
                            job_id))
                else:
    ...
    def map_status(cls, status):
        # see https://pbs.schedmd.com/squeue.html#lbAG
        status = status.strip("+")
        if status in ["CONFIGURING", "PENDING", "REQUEUED", "REQUEUE_HOLD", "REQUEUE_FED"]:
            ...
            logger.debug("unknown pbs job state '{}'".format(status))
            return cls.FAILED


class PbsJobFileFactory(BaseJobFileFactory):
    ...
    def __init__(self, file_name="pbs_job.sh", command=None, executable=None, arguments=None,
        ...
        if kwargs.get("dir") is None:
            kwargs["dir"] = cfg.get_expanded("job", cfg.find_option("job",
                "pbs_job_file_dir", "job_file_dir"))
        if kwargs.get("mkdtemp") is None:
            kwargs["mkdtemp"] = cfg.get_expanded_bool("job", cfg.find_option("job",
                "pbs_job_file_dir_mkdtemp", "job_file_dir_mkdtemp"))
        if kwargs.get("cleanup") is None:
            kwargs["cleanup"] = cfg.get_expanded_bool("job", cfg.find_option("job",
                "pbs_job_file_dir_cleanup", "job_file_dir_cleanup"))

        super(PbsJobFileFactory, self).__init__(**kwargs)
    ...
    def create(self, postfix=None, **kwargs):
        ...
        logger.debug("created pbs job file at '{}'".format(job_file))
